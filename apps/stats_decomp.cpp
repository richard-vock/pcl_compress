...
main(int argc, char const* argv[]) {
    ...
    for (uint32_t scan_idx = 0; scan_idx < scan_count; ++scan_idx) {
        ...
        pcl_compress::decomposition_t primitive_sets;
        uint32_t primitive_patch_count;
        decomposition_t decomp = primitive_decomposition<point_normal_t>(
            cloud_in, params, max_points, 6, 0.2f/*0.03f * bbox.diagonal().norm()*/, &primitive_sets, &primitive_patch_count);

        std::cout << "\t" << primitive_patch_count << " / " << decomp.size() << std::endl;
