...
int main (int argc, char const* argv[]) {
    ...
    for (auto it = octree.leaf_begin(); it != octree.leaf_end(); ++it) {
        ...
        patch_t patch = compute_patch(cloud_in, subset, 100.f, 1E-4);
    ...
    uint32_t chars = 4 * 4 + 2*cc->origins.size() + 2*cc->bboxes.size() + cc->bases.size() + cc->global_occ_data->length;
    for (const auto& chunk : cc->patch_image_data) {
        chars += chunk->length;
    }
    std::cout << cloud_in->size() * 12 << "\n";
    std::cout << chars << "B" << "\n";
    std::cout << (chars / 1024) << "kB" << "\n";
