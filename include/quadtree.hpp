...
class quadtree::node {
    ...
    public:
        node(const bbox2f_t& bbox, const std::vector<vec2f_t>& points, const indices_t& subset, const params_t& params, uint32_t depth = 0);
        virtual ~node();
    ...
    protected:
        static bool inside_(uint32_t quadrant, const vec2f_t& point, const vec2f_t& center);
